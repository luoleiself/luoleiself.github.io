...
对象存储是一种将数据作为对象进行管理的存储架构，每个对象包含数据本身、扩展元数据以及一个全局唯一标识符. 它通过扁平化命名空间直接访问对象，消除了传统文件系统的层级目录结构.

- Bucket(桶), 对象的顶层容器, 通常有区域、访问策略、生命周期规则
- Key(键), 对象在桶内的逻辑路径, 如 images/2026/pyoto.png (只是字符串, 不必真有文件夹)
- 版本控制, 同一 Key 可保留多版本, 便于误删恢复与合规
- 预签名 URL, 临时授权下载/上传, 不把长期密钥给客户端
- 生命周期, 转冷存储、过期删除、分段上传清理等
- 多部分上传, 大文件分片上传与合并
...
- \-\-cpuset-cpus string  绑定 cpu, `--cpuset-cpus '0,2,4,6'`
...
```bash
docker run -dit -p 6666:80 --name nginx01 --cpuset-cpus '0,2,4,6' --log-driver gelf nginx
```
