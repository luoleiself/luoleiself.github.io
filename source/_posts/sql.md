...
- $collStats  返回有关集合或视图的统计信息, 必须是聚合管道中的第一阶段
- $count 计数
...
- $merge  将 aggregation pipeline 的结果文档写入集合, 必须是聚合管道中的最后一个阶段
- $out  将 aggregation pipeline 的结果文档写入集合, 必须是聚合管道中的最后一个阶段
- $planCacheStats 返回集合的计划缓存的信息, 必须是聚合管道中的第一阶段
