...
List 是一个有序重复的字符串列表, 按照添加的顺序排序, 可以添加一个元素到列表的头部(左边)、尾部(右边), 一个列表最多可以包含 2^32-1(40 多亿) 个元素.
...
(integer) 5
127.0.0.1:6379> LRANGE list 0 100 # 遍历 list
1) "hello"
2) "a"
3) "d"
4) "c"
...
- BLPOP key [key...] timeout 从多个列表中第一个非空列表中的头部移除并返回一个元素, 如果列表为空会阻塞列表直到等待超时或发现可弹出元素为止, 如果列表为空或者超时返回 &lt;nil&gt;
  否则, 返回一个含有两个元素的列表, 第一个元素是被弹出元素所属的列表, 第二个元素是被弹出的元素
- BRPOP key [key ...] timeout 从多个列表中第一个非空列表中的尾部移除并返回一个元素, 返回值 `BLPOP`
...
- LMPOP numkeys key [key ...] LEFT|RIGHT [COUNT count] 从多个列表中第一个非空列表中指定位置批量移除指定数量的元素并返回操作成功的 key 和移除的元素, 如果列表都为空或者不存在返回 &lt;nil&gt;, 7.0.0 支持
...
(integer) 6
127.0.0.1:6379> LMPOP 2 list newlist LEFT COUNT 3 # list 不为空, 从 list 的头部删除返回 3 个 元素
1) "list"
2) 1) "a"
   2) "b"
   3) "c"
...
(integer) 1
127.0.0.1:6379> LMPOP 2 list newlist right COUNT 3  # list 为空, 从 newlist 的尾部删除返回 3 个元素
1) "newlist"
2) 1) "6"
   2) "5"
   3) "4"
...
"a"
127.0.0.1:6379> LRANGE newlist 0 4  # 遍历 newlist
1) "1"
2) "2"
3) "3"
4) "a"
...
"a"
127.0.0.1:6379> LRANGE newlist 0 100  # 遍历 newlist
1) "a"
