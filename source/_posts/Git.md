...
> 撤销工作区, 暂存区, 版本库的修改使用 restore 命令

- git checkout -- [file]   # 撤销工作区指定文件修改
- git checkout . # 撤销工作区所有文件修改
- git checkout [commit] [file]   # 恢复某个 `commit` 的指定文件到工作区
- git checkout <branch> -- <file>  # 从其它分支恢复文件

![git-checkout-1.png](/images/git-checkout-1.png)

### reset

- git reset HEAD -- \<file\>  # 撤销暂存区指定文件修改到工作区
...
- git reset --hard # 重置版本库、暂存区、工作区，与上一次 `commit` 保持一致
...
- -s,--source=\<commit\> 从指定的提交中恢复文件到工作区(暂存区和版本库不变)。默认为 HEAD，即当前提交
...
# 想用历史版本的文件作为新提交
git restore -s HEAD~1 --staged test.txt
git commit -m '回滚 test.txt 到上一个版本'

# 从指定提交同时恢复工作区和暂存区
# 不加 --staged 只恢复到工作区, 暂存区不变
# 加 --staged 只恢复到暂存区, 工作区不变
git restore -s HEAD~1 --staged --worktree test.txt
```
...
- git stash # 将没有提交的内容(包含工作区和暂存区)保存至堆栈中
  ...
  - push [[--patch] [-a|--all] [-- \<pathspec\>...]]  # 将指定的文件或者使用模式匹配到的文件存入堆栈
  ```bash
  git stash push .env.dev src/test/*.ts  # 将指定文件存入堆栈
  ```
...
- git commit --amend --no-edit # 使用上次的提交不修改提交信息
