...
React 在每次渲染时创建的本次渲染的状态副本, 每个渲染都有自己独立的 props、state、事件处理函数等, 它们都是本次渲染的常量
...
- setup, 处理 Effect 的函数
  - 选择性返回一个清理(cleanup) 函数. 当组件被添加(挂载)到 DOM 时, React 将运行 setup 函数.
  - 在每次依赖项变更重新渲染后, React 将首先使用旧值运行 cleanup 函数，然后使用新值运行 setup 函数.
  - 在组件从 DOM 中移除(卸载)后, React 将最后一次运行 cleanup 函数.
...
> 凡是函数内部使用到的、在函数外部声明的、且可能变化的变量，都必须包含在依赖数组中
