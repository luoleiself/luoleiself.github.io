...
- ssh [参数] [用户@]主机名 [命令]
  - \-p 端口号
  - \-l 用户名, 指定登录远程服务器的用户名, 默认使用本地登录的用户名
  - \-i 私钥文件名, 指定用于身份认证的私钥文件路径
  - \-L 本地端口:目标主机:目标端口 用户名@目标地址, 实现本地端口转发, 将本地电脑上的某个端口映射到远程服务器上的某个服务
  - \-R 远程端口:目标主机:目标端口 用户名@目标地址, 实现远程(反向)端口转发, 将远程服务器上的某个端口映射到本地电脑上的服务
  - \-A 开启代理转发功能, 允许在远程主机上使用本地电脑上的 SSH 代理进行身份验证
  - \-N 不执行远程命令, 仅用于建立 SSH 连接
  - \-f 将 SSH 会话放入后台运行
  - \-T 禁用伪终端分配
  - \-t 强制分配伪终端
  - \-C 启用数据压缩
  - \-X 开启 X11 转发功能
  - \-Y 启用受信任的 X11 转发
  - \-q 静默模式, 减少 SSH 命令返回的输出信息, 仅显示关键错误或核心信息

```bash
# 指定用户名连接: 同 user@address 格式
ssh -l admin 192.168.1.100

# ssh -L [本地端口]:[目标主机]:[目标端口] [用户名]@[目标地址]

# 本地端口转发在本地访问 localhost:8080 代理远程服务器上的 80 端口服务
ssh -L 8080:localhost:80 admin@192.168.1.100

# 远程端口转发在服务器访问 192.168.1.100:9090 代理本地的 90 端口服务
ssh -R 9090:localhost:90 admin@192.168.1.100

# 仅建立隧道不登录(后台运行)
ssh -N -f -L 8080:localhost:80 admin@192.168.1.100

# 脚本自动化执行
ssh -q admin@192.168.1.100 "ls /home"
