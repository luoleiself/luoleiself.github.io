...
但这些数据库并不是物理隔离的, 它们共享一个进程和内存, 因此不适合用于多租户或强隔离场景.
在生产环境中, Redis 官方和社区更推荐使用单库, 通过 key 前缀或多实例的方式进行隔离.
同时, 在 Redis Cluster 模式下不支持多数据库.
```
...
# 批量执行 redis 命令
[root@centos7 workspace]# cat multi.txt | redis-cli --pipe
```
