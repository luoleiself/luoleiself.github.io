...
print(name_dict['forth'])  # key 不存在时会报错
# Traceback (most recent call last):
#   File "<stdin>", line 1, in <module>
# KeyError: 'forth'
print('forth' in name_dict)  # in 检查 key 是否存在
# False
...
print(dt)
# {'a': 'A', 'b': 'B'}
print(dt.setdefault('a'))  # 键存在则返回对应的值
'A'
print(dt.setdefault('a', 1))  # 键存在则返回对应的值, 默认值不会影响已存在的键
'A'
print(dt.setdefault('c'))  # 添加键, 未提供默认值则使用 None
# None
print(dt)
# {'a': 'A', 'b': 'B', 'c': None}
print(dt.setdefault('d', 'D'))  # 添加键, 提供默认值并返回默认值
'D'
print(dt)
# {'a': 'A', 'b': 'B', 'c': None, 'd': 'D'}
```
...
# {'a': 'good', 'b': 'good', 1: 'good'}
dict.fromkeys({'a', 'b', 1}, 'good')  # 根据 set 的值生成 dict
...
- pop(key, default) 按 key 删除字典键值对并返回删除的值
    - key, 要删除的键
