...
异常处理
    ...
    - @app.exception_handler()   # 添加异常处理函数装饰器
        - RequestValidationError # 内部默认请求包含无效数据异常
        - HTTPException

    @app.exception_handler(UnicornException)    # 注册 unicorn 异常处理器
    async def unicorn_exception_handler(request: Request, exc: UnicornException):
        return JSONResponse(status_code=418, content={'message': f'{exec.name} did something')

中间件(middleware): 是一个函数, 在每个特定的路径操作函数处理每个请求之前运行, 在返回每个响应之前运行.
    - app.add_middleware():   # 添加中间件
    - @app.middleware('http') # 装饰器添加 http 中间件, 按定义顺序自下而上执行
        request: 请求实例
...
SSE: 
    - 路径装饰器参数设置: response_class=EventSourceResponse, 
      路径操作函数中使用 yield
    - 需要配置 event, id, retry, comment 等 sse 字段时, 使用 yield ServerSentEvent 对象
        ServerSentEvent(data={}, event='event', id=str(i + 1), retry=1000)
...
pydantic: 数据校验库, 定义包含属性的继承 BaseModel 的类, 实例化这个类会自动校验这些属性值, 
    ...
    BaseModel: 所有模型校验和序列化操作的基类, 用于定义拥有明确、固定字段的数据结构
    RootModel: 用于给数据本身添加验证和序列化能力，而不是给它的字段, 比如纯列表 [1, 2, 3], 纯字典{'a': 1}, 或任意单一层级的原始值,
        模型本身包装了一个根数据，通过 root 属性来访问这个核心值.
    ...
    AliasPath: 字段别名
    Field: 在 pydantic 模型内部声明校验和元数据, 以提供有关字段和验证的额外信息.
    SecretStr:   # 安全的字符串表示类
        s = SecretStr('sk-abcdefg')
    ...
    序列化(Serializers): 可以针对整个模型进行定制, 也可以基于每个字段或每个类型进行定制
        装饰器序列化:
            @model_serializer(mode='plain') # 模型序列化
            @field_serializer(field_name, mode='plain') # 字段序列化
            ...
            class NumberModel(BaseModel):
                ...
                def serialize_model(self) -> str:
                    return f'number: {self.number} - good!'
            
            print(NumberModel(number=10).model_dump())
            #> number: 20 - good
        注解序列化:
            WrapSerializer: 包装序列化, 提供更大的灵活性来定制序列化行为, 可以在序列化逻辑之前或之后运行代码
            PlainSerializer: 普通序列化
    ...
    验证器(Validators):
        装饰器验证器:
            @validate_call  # 允许在调用函数之前使用函数的注释解析和验证传递给函数的参数.
                @validate_call
            ...
            @model_validator(mode='before' | 'after' | 'wrap')  # 模型校验装饰器
            @field_validator(field_name, mode='before' | 'after' | 'wrap' | ‘plain’)    # 字段校验装饰器
        注解验证器:
    ...
    print(user)
    # User id=123 name='John Doe' signup_ts=datetime(2025, 3, 29, 16, 27, 30) friends=[1, 2, 3]
    # 校验数据结构
...
m = BarModel(foo=datetime.now(timezone.utc), bar='bar')
...
class IndexResponseModel(CookieModel, HeaderModel):
    version: str = Field(serialization_alias='_version')  # 设置序列化后的字段名
...
@app.get('/stream/sse', response_class=EventSourceResponse, tags=['stream'])
async def sse_items():
    """
    sse:
        1. 设置 response_class=EventSourceResponse, # 从 fastapi.sse 导入 EventSourceResponse
        2. 在路径操作函数中使用 yield, 每个被 yield 的项会被编码为 JSON, 并放入 sse 事件的 data: 字段发送
        3. 如果需要设置 event, id, retry, comment 等 sse 字段时, 可以 yield ServerSentEvent 对象, 而不是直接返回数据
    """
    for i in range(20):
        # yield {'id': i, 'name': f'name-{i}'}
        yield ServerSentEvent(data={'id': i, 'name': f'name-{i}', 'comment': 'ServerSentEvent'}, event='event',
                              id=str(i + 1), retry=1000)
...
def exception_handler(request: Request, exc: Exception):
    print(f'request: {request}, exc: {exc}')
