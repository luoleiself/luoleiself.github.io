...
class AsyncContextManager:
    async def __aenter__(self):
        print('AsyncContextManager entering context')
        await asyncio.sleep(1)
        return self

    async def __aexit__(self, exc_type, exc, tb):
        print('AsyncContextManager exiting context')
        await asyncio.sleep(1)

# 使用异步上下文装饰器和 yield
@asynccontextmanager
async def async_context_manager():
    print('async_context_manager entering context')
    await asyncio.sleep(1)
    yield {}
    print('async_context_manager exiting context')
    await asyncio.sleep(1)
        

async def main():
    # async for 和 async with 必须在 async 函数内执行, 否则会报错
