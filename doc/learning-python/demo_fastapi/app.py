...
# 配置应用位置
[tool.fastapi]
entrypoint = 'main:app' # from main import app
...
@app.get('/news/list')
async def get_news(commons: Annotated[dict, Depends(depend_function)]):
    return commons


@app.get('/blog/list')
async def get_blog(commons: Annotated[dict, Depends(depend_function)]):
    return commons
